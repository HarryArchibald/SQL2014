...
.\Setup.exe /ACTION=Install /IAcceptSQLServerLicenseTerms /QS /IndicateProgress=True /UpdateEnabled=False /UpdateSource=MU /Features=SQLENGINE,CONN,SSMS,ADV_SSMS /InstallSharedDir=$INSTALLSHAREDDIR /InstallSharedWowDir=$INSTALLSHAREDWOWDIR /InstanceName=$InstanceName /InstanceID=$InstanceName /InstanceDir=$INSTANCE_DIR /AGTSVCACCOUNT="$Domain\$AgentAccount" /AGTSVCPASSWORD="$AgentPassword" /AGTSVCSTARTUPTYPE=Automatic /BROWSERSVCSTARTUPTYPE=Disabled /SAPWD="$SAPassword" /SQLBACKUPDIR=$BACKUPDIR /SQLUSERDBDIR=$USERDBDIR /SQLUSERDBLOGDIR=$LOGDIR /InstallSQLDataDIR=$INSTALLSQLDATADIR /SQLTEMPDBDIR=$SQLTEMPBDIR /SQLTEMPDBLOGDIR=$SQLTEMPDBLOGDIR /SQLSVCACCOUNT="$Domain\$SQLAccount" /SQLSVCPASSWORD="$SQLPassword" /SQLSVCSTARTUPTYPE=Automatic /SQLSYSADMINACCOUNTS="$Domain\$SysAdminADGroup" /TCPENABLED=1 /NPENABLED=0 /SQLCOLLATION="$Collation" 